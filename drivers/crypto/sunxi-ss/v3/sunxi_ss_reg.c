...
void ss_key_set(char *key, int size, ce_task_desc_t *task)
{
	...
	task->key_addr = (virt_to_phys(key)) >> WORD_ALGIN;
}
...
{
	task->iv_addr = virt_to_phys(iv) >> WORD_ALGIN;
}
...
{
	task->ctr_addr = virt_to_phys(cnt) >> WORD_ALGIN;
...
void ss_ctrl_start(ce_task_desc_t *task)
{
	...
	ss_writel(CE_REG_TSK, (virt_to_phys(task) >> WORD_ALGIN));
#ifdef SS_SUPPORT_CE_V3_1
