...
static int sensor_helper_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(sensor_helper->pmic[IOVDD])) {
		vin_warn("%s: cannot get IOVDD supply, setting it to NULL!\n", __func__);
	...
	if (IS_ERR(sensor_helper->pmic[AVDD])) {
		vin_warn("%s: cannot get AVDD supply, setting it to NULL!\n", __func__);
	...
	if (IS_ERR(sensor_helper->pmic[DVDD])) {
		vin_warn("%s: cannot get DVDD supply, setting it to NULL!\n", __func__);
...
MODULE_DESCRIPTION("Camera sensor DRIVER for sunxi");
#endif
