...
static void axp2101_dts_parse(struct axp20x_dev *axp20x)
{
	...
	if (of_property_read_bool(axp20x->dev->of_node, "pmu_watchdog_en")) {
		regmap_update_bits(axp20x->regmap, AXP2101_MODULE_EN, BIT(0),
				   BIT(0));
		if (!of_property_read_u32(axp20x->dev->of_node,
					 "pmu_watchdog_cfg", &out_value)) {
			regmap_write(axp20x->regmap, AXP2101_WATCHDOG_CFG,
