...
int add_nand(struct nand_blk_ops *tr, struct _nand_phy_partition *phy_partition)
{
	...
	panic_init_part(NULL, 0, 0, nftl_blk);
	...
	for (i = 0; i < MAX_PART_COUNT_PER_FTL; i++) {
		...
		if (disk[i].name[0] == 0xFF && disk[i].name[1] == 0xFF)
			break;
		panic_init_part(disk[i].name, partition_total_sector, disk[i].size, NULL);
...
{
    return panic_read(dev_name, start_sector, sector_num, buf);
}
EXPORT_SYMBOL(critical_dev_nand_read);
...
{
    return panic_write(dev_name, start_sector, sector_num, buf);
}
EXPORT_SYMBOL(critical_dev_nand_write);
